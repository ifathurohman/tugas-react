...
import {getInvoiceByOrderId, getOrdersDetail} from '../app/api/order';
          ...
          <div className="max-w-2xl mx-auto py-16 px-4 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8 lg:py-32 lg:grid lg:grid-cols-2 lg:gap-x-8 xl:gap-x-24">
            <div className="lg:col-start-2">
              ...
              <dl className="mt-16 text-sm font-medium">
                ...
                <dd className="mt-2 text-indigo-600">{invoice?._id}</dd>
              </dl>

              <ul
                role="list"
                className="mt-6 text-sm font-medium text-gray-500 border-t border-gray-200 divide-y divide-gray-200">
                {orderDetail?.order_items?.map((order, key) => (
                  <li key={key} className="flex py-6 space-x-6">
                    <img
                      src={
                        config.api_host + '/images/products/' + order?.image_url
                      }
                      alt={order?.name}
                    ...
                    <div className="flex-auto space-y-1">
                      <h3 className="text-gray-900">
                        <a href={`/product/${order.name}`}>{order.name}</a>
                      </h3>
                    ...
                    <p className="flex-none font-medium text-gray-900">
                      <FormatRupiah value={order.price * order.qty} />
                    </p>
                  </li>
                ))}
              </ul>
                ...
                <div>
                  ...
                  <dd className="mt-2">
                    <address className="not-italic">
                      <span className="block">{detailUser?.full_name}</span>
                ...
                <div>
                  ...
                  <dd className="mt-2 space-y-2 sm:flex sm:space-y-0 sm:space-x-4">
                    <div className="flex-none">
                      ...
                      <p className="sr-only">{found.va_numbers[0]?.bank}</p>
                    </div>
                    <div className="flex-auto">
                      <p>{moment(found?.transaction_time).format('LLL')}</p>
                      <p className="text-gray-900 uppercase">
                        {found.va_numbers[0]?.bank}
                      </p>
                      <p>{found.va_numbers[0]?.va_number}</p>
                      <div className="w-24 p-1 relative flex flex-col sm:items-center bg-green-200 shadow rounded-md">
                        <p className="uppercase text-green-500 font-semibold text-center">
                          {found.transaction_status}
                        </p>
                      </div>
                    </div>
                  </dd>
